...
#define CONSOLE_LOG_HEIGHT                                                     \
    (FONT_HEIGHT_SCALE * FONT_CHAR_HEIGHT * CONSOLE_LOG_CAPACITY)
...
Console *create_console(Game *game)
{
    ...
    console->console_log = PUSH_LT(lt,
        create_console_log(
            vec(FONT_WIDTH_SCALE, FONT_HEIGHT_SCALE), CONSOLE_LOG_CAPACITY),
        destroy_console_log);

    console->a = 0;

    console->history =
        PUSH_LT(lt, create_history(HISTORY_CAPACITY), destroy_history);
...
static int console_eval_input(Console *console)
{
    ...
    if (console_log_push_line(
            console->console_log, input_text, NULL, CONSOLE_FOREGROUND)
        < 0) {
        return -1;
    }
    ...
    if (string_equal(command, STRING_LIT("load"))) {
        ...
        console_log_push_line(
            console->console_log, "Loading level:", NULL, CONSOLE_FOREGROUND);
        console_log_push_line(console->console_log,
            level.data,
            level.data + level.count,
            CONSOLE_FOREGROUND);
        char level_name[256];
        ...
        console_log_push_line(
            console->console_log, "Loading menu", NULL, CONSOLE_FOREGROUND);
        game_switch_state(console->game, GAME_STATE_LEVEL_PICKER);
    } else {
        console_log_push_line(
            console->console_log, "Unknown command", NULL, CONSOLE_ERROR);
    }
...
int console_handle_event(Console *console, const SDL_Event *event)
{
    switch (event->type) {
    case SDL_KEYDOWN: {
...
int console_render(const Console *console, const Camera *camera)
{
    ...
    if (camera_fill_rect_screen(camera,
            rect(0.0f, y, (float)view_port.w, CONSOLE_HEIGHT),
            CONSOLE_BACKGROUND)
        < 0) {
        return -1;
    }

    console_log_render(console->console_log, camera, vec(0.0f, y));

    if (edit_field_render_screen(
            &console->edit_field, camera, vec(0.0f, y + CONSOLE_LOG_HEIGHT))
        < 0) {
        return -1;
    }
