...
{
    const Triangle result = { .p1 = p1, .p2 = p2, .p3 = p3 };
...
Triangle equilateral_triangle(void)
{
    ...
    const Triangle result = { .p1 = vec(cosf(0.0f), sinf(0.0f)),
...
{
    return triangle(vec_from_polar(rand_float(2 * PI), rand_float(radius)),
...
{
    Triangle t1 = { .p1 = { .x = rect.x, .y = rect.y },
        .p2 = { .x = rect.x + rect.w, .y = rect.y },
        .p3 = { .x = rect.x, .y = rect.y + rect.h } };

    Triangle t2 = { .p1 = { .x = rect.x + rect.w, .y = rect.y },
