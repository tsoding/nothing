...
int draw_triangle(SDL_Renderer *render, Triangle t)
{
    trace_assert(render);
...
static int fill_bottom_flat_triangle(SDL_Renderer *render, Triangle t)
{
    trace_assert(render);
...
static int fill_top_flat_triangle(SDL_Renderer *render, Triangle t)
{
    trace_assert(render);
...
int fill_triangle(SDL_Renderer *render, Triangle t)
{
    ...
    if (fabs(t.p2.y - t.p3.y) < 1e-6) {
        ...
        const Vec2f p4 = vec(t.p1.x
                + ((t.p2.y - t.p1.y) / (t.p3.y - t.p1.y)) * (t.p3.x - t.p1.x),
            t.p2.y);
...
int fill_rect(SDL_Renderer *render, Rect r, Color c)
{
    ...
    if (SDL_SetRenderDrawColor(
            render, sdl_color.r, sdl_color.g, sdl_color.b, sdl_color.a)
        < 0) {
