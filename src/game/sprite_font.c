...
static inline int scc(int code)
{
    if (code < 0) {
        log_fail("SDL error: %s\n", SDL_GetError());
        trace_assert(0 && "SDL error");
...
SDL_Texture *load_bmp_font_texture(
    SDL_Renderer *renderer, const char *bmp_file_path)
{
    ...
    scc(SDL_SetColorKey(
        surface, SDL_TRUE, SDL_MapRGB(surface->format, 0, 0, 0)));

    SDL_Texture *result = scp(SDL_CreateTextureFromSurface(renderer, surface));
...
static SDL_Rect sprite_font_char_rect(const Sprite_font *sprite_font, char x)
{
    ...
    if (32 <= x && x <= 126) {
        const SDL_Rect rect = { .x = ((x - 32) % FONT_ROW_SIZE)
                * FONT_CHAR_WIDTH,
    ...
    const char *text)
{
    ...
    scc(SDL_SetTextureColorMod(
        sprite_font->texture, sdl_color.r, sdl_color.g, sdl_color.b));
    scc(SDL_SetTextureAlphaMod(sprite_font->texture, sdl_color.a));
    ...
    for (size_t i = 0, col = 0, row = 0; i < text_size; ++i) {
        ...
        const SDL_Rect dest_rect = rect_for_sdl(
            rect(position.x + (float)FONT_CHAR_WIDTH * (float)col * size.x,
                position.y + (float)FONT_CHAR_HEIGHT * (float)row * size.y,
                (float)char_rect.w * size.x,
                (float)char_rect.h * size.y));
        scc(SDL_RenderCopy(
            renderer, sprite_font->texture, &char_rect, &dest_rect));
        col++;
    }
}
