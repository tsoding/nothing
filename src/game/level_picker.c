...
#define ITEM_HEIGHT                                                            \
    (FONT_CHAR_HEIGHT * LEVEL_PICKER_LIST_FONT_SCALE.y                         \
        + LEVEL_PICKER_LIST_PADDING_BOTTOM)

#define SCROLLBAR_WIDTH 20
#define SCROLLING_SPEED_FRACTION 0.25f

void level_picker_populate(LevelPicker *level_picker, const char *dirpath)
{
...

    {
        ...
        level_picker->items =
            create_dynarray_malloc(METADATA_FILEPATH_MAX_SIZE);
        ...
        for (struct dirent *d = readdir(level_dir); d != NULL;
             d = readdir(level_dir)) {
            if (*d->d_name == '.')
                continue;

            snprintf(filepath,
                METADATA_FILEPATH_MAX_SIZE,
                "%s/%s",
                dirpath,
                d->d_name);
...
int level_picker_render(const LevelPicker *level_picker, const Camera *camera)
{
    trace_assert(level_picker);
    ...
    const float scrolling_area_height =
        viewport.h - ITEM_HEIGHT - level_picker->items_position.y;

    wiggly_text_render(&level_picker->wiggly_text,
        camera,
        vec(viewport.w * 0.5f - title_size.x * 0.5f, TITLE_MARGIN_TOP));

    const float proportional_scroll = level_picker->items_scroll.y
        * scrolling_area_height / level_picker->items_size.y;
    const float number_of_items_in_scrolling_area =
        scrolling_area_height / ITEM_HEIGHT;
    const float percent_of_visible_items = number_of_items_in_scrolling_area
        / ((float)level_picker->items.count - 1);

    if (level_picker->items.count > 0 && percent_of_visible_items < 1) {
        SDL_Rect scrollbar = rect_for_sdl(rect_from_vecs(
            vec(level_picker->items_position.x + level_picker->items_size.x,
                level_picker->items_position.y),
            vec(SCROLLBAR_WIDTH, scrolling_area_height)));

        SDL_Rect scrollbar_thumb = rect_for_sdl(rect_from_vecs(
            vec(level_picker->items_position.x + level_picker->items_size.x,
                level_picker->items_position.y - proportional_scroll),
            vec(SCROLLBAR_WIDTH,
                scrolling_area_height * percent_of_visible_items)));
    ...
    for (size_t i = 0; i < level_picker->items.count; ++i) {
        const Vec2f current_position = vec_sum(level_picker->items_position,
            vec(0.0f, (float)i * ITEM_HEIGHT + level_picker->items_scroll.y));

        if (current_position.y
                > level_picker->items_position.y + scrolling_area_height
        ...
        if (i == level_picker->items_cursor) {
            SDL_Rect boundary_box = rect_for_sdl(sprite_font_boundary_box(
                current_position, LEVEL_PICKER_LIST_FONT_SCALE, item_text));
            if (SDL_SetRenderDrawColor(camera->renderer, 255, 255, 255, 255)
                < 0) {
                return -1;
            }
...

    {
        ...
        const Vec2f position =
            vec(0.0f, viewport.h - size.y * FONT_CHAR_HEIGHT);
        ...
        camera_render_text_screen(camera,
            ...
            size,
            COLOR_WHITE,
            vec(position.x + padding, position.y - padding));
    }

    return 0;
}

int level_picker_update(
    LevelPicker *level_picker, Camera *camera, float delta_time)
{
    trace_assert(level_picker);
    ...
    if ((float)level_picker->items_cursor * ITEM_HEIGHT
            + level_picker->items_scroll.y
    ...
    vec_add(&level_picker->camera_position, vec(50.0f * delta_time, 0.0f));
...
static Vec2f level_picker_list_size(const LevelPicker *level_picker)
{
    trace_assert(level_picker);
    ...
    for (size_t i = 0; i < level_picker->items.count; ++i) {
        ...
        Rect boundary_box = sprite_font_boundary_box(
            vec(0.0f, 0.0f), LEVEL_PICKER_LIST_FONT_SCALE, item_text);
...
int level_picker_event(LevelPicker *level_picker, const SDL_Event *event)
{
    ...
    case SDL_WINDOWEVENT: {
        ...
        case SDL_WINDOWEVENT_SIZE_CHANGED: {
            ...
            SDL_GetRendererOutputSize(
                SDL_GetRenderer(SDL_GetWindowFromID(event->window.windowID)),
                &width,
                NULL);
            const Vec2f title_size =
                wiggly_text_size(&level_picker->wiggly_text);
    ...
    case SDL_MOUSEBUTTONDOWN: {
        ...
        case SDL_BUTTON_LEFT: {
            ...
            Vec2f position = vec_sum(
                level_picker->items_position, level_picker->items_scroll);

            for (size_t i = 0; i < level_picker->items.count; ++i) {
                const char *item_text =
                    dynarray_pointer_at(&level_picker->items, i);

                Rect boundary_box = sprite_font_boundary_box(
                    position, LEVEL_PICKER_LIST_FONT_SCALE, item_text);
    ...
    case SDL_MOUSEBUTTONUP: {
        ...
        case SDL_BUTTON_LEFT: {
            ...
            vec_add(&position,
                vec(0.0f,
                    (float)level_picker->items_cursor * single_item_height));

            const char *item_text = dynarray_pointer_at(
                &level_picker->items, level_picker->items_cursor);

            Rect boundary_box = sprite_font_boundary_box(
                position, LEVEL_PICKER_LIST_FONT_SCALE, item_text);

            const Vec2f mouse_pos =
                vec((float)event->motion.x, (float)event->motion.y);
            if (rect_contains_point(boundary_box, mouse_pos)) {
                level_picker->selected_item = (int)level_picker->items_cursor;
            }
        } break;
...
const char *level_picker_selected_level(const LevelPicker *level_picker)
{
    ...
    return dynarray_pointer_at(
        &level_picker->items, (size_t)level_picker->selected_item);
}
...
int level_picker_enter_camera_event(LevelPicker *level_picker, Camera *camera)
{
