...
static LabelUndoContext create_label_undo_swap_context(
    LabelLayer *label_layer, size_t index, size_t index2)
{
...
static LabelUndoContext create_label_undo_context(
    LabelLayer *label_layer, LabelUndoType type)
{
    trace_assert(label_layer);
    ...
    size_t index = type == LABEL_UNDO_ADD ? label_layer->positions.count - 1
...
static void label_layer_undo(void *context, size_t context_size)
{
    ...
    case LABEL_UNDO_DELETE: {
        dynarray_insert_before(
            &label_layer->ids, undo_context->index, &undo_context->id);
        dynarray_insert_before(&label_layer->positions,
            undo_context->index,
            &undo_context->position);
        dynarray_insert_before(
            &label_layer->colors, undo_context->index, &undo_context->color);
        dynarray_insert_before(
            &label_layer->texts, undo_context->index, &undo_context->text);
    } break;

    case LABEL_UNDO_UPDATE: {
        dynarray_replace_at(
            &label_layer->ids, undo_context->index, &undo_context->id);
        dynarray_replace_at(&label_layer->positions,
            undo_context->index,
            &undo_context->position);
        dynarray_replace_at(
            &label_layer->colors, undo_context->index, &undo_context->color);
    ...
    case LABEL_UNDO_SWAP: {
        dynarray_swap(
            &label_layer->ids, undo_context->index, undo_context->index2);
        dynarray_swap(
            &label_layer->positions, undo_context->index, undo_context->index2);
        dynarray_swap(
            &label_layer->colors, undo_context->index, undo_context->index2);
        dynarray_swap(
            &label_layer->texts, undo_context->index, undo_context->index2);
    } break;
    }
}
...
#define LABEL_UNDO_PUSH(HISTORY, CONTEXT)                                      \
    do {                                                                       \
        ...
        undo_history_push(                                                     \
            HISTORY, label_layer_undo, &context, sizeof(context));             \
    } while (0)

LayerPtr label_layer_as_layer(LabelLayer *label_layer)
{
    LayerPtr layer = { .ptr = label_layer, .type = LAYER_LABEL };
    return layer;
}
...
LabelLayer *create_label_layer(Memory *memory, const char *id_name_prefix)
{
    ...
    result->ids =
        create_dynarray(memory, sizeof(char) * LABEL_LAYER_ID_MAX_SIZE);
    result->positions = create_dynarray(memory, sizeof(Vec2f));
    result->colors = create_dynarray(memory, sizeof(Color));
    result->texts =
        create_dynarray(memory, sizeof(char) * LABEL_LAYER_TEXT_MAX_SIZE);
...
void label_layer_load(LabelLayer *label_layer, Memory *memory, String *input)
{
    trace_assert(label_layer);
    trace_assert(memory);
    trace_assert(input);
    ...
    for (int i = 0; i < n; ++i) {
        ...
        Vec2f position;
        position.x =
            strtof(string_to_cstr(memory, trim(chop_word(&meta))), NULL);
        position.y =
            strtof(string_to_cstr(memory, trim(chop_word(&meta))), NULL);
        ...
        String label_text_string = trim(chop_by_delim(input, '\n'));
        memset(label_text, 0, LABEL_LAYER_TEXT_MAX_SIZE);
        memcpy(label_text,
            label_text_string.data,
            min_size_t(LABEL_LAYER_TEXT_MAX_SIZE - 1, label_text_string.count));
...
static inline Rect boundary_of_element(
    const LabelLayer *label_layer, size_t i, Vec2f position)
{
    ...
    return rect_boundary2(
        sprite_font_boundary_box(
            position, LABELS_SIZE, texts + i * LABEL_LAYER_TEXT_MAX_SIZE),
        sprite_font_boundary_box(
            vec_sum(
                position, vec_mult(vec(0.0f, FONT_CHAR_HEIGHT), LABELS_SIZE)),
            vec(1.0f, 1.0f),
            ids + i * LABEL_LAYER_ID_MAX_SIZE));
}

int label_layer_render(
    const LabelLayer *label_layer, const Camera *camera, int active)
{
    trace_assert(label_layer);
    trace_assert(camera);
    ...
    for (size_t i = 0; i < n; ++i) {
        const Color color = label_layer->state == LABEL_LAYER_RECOLOR
                && label_layer->selection == (int)i
            ? label_layer->inter_color
            : colors[i];

        const Vec2f position = label_layer->state == LABEL_LAYER_MOVE
                && label_layer->selection == (int)i
        ...
        if (label_layer->state == LABEL_LAYER_EDIT_TEXT
            ...
            if (edit_field_render_world(
                    &label_layer->edit_field, camera, position)
                < 0) {
                return -1;
            }
        } else {
            if (camera_render_text(camera,
                    ...
                    LABELS_SIZE,
                    color_scale(
                        color, rgba(1.0f, 1.0f, 1.0f, active ? 1.0f : 0.5f)),
                    position)
                < 0) {
                return -1;
            }
        ...
        if (label_layer->state == LABEL_LAYER_EDIT_ID
            && label_layer->selection == (int)i) {
            if (edit_field_render_world(&label_layer->edit_field,
                    camera,
                    vec_sum(position,
                        vec_mult(vec(0.0f, FONT_CHAR_HEIGHT), LABELS_SIZE)))
                < 0) {
                return -1;
            }
        } else {
            if (camera_render_text(camera,
                    ids + i * LABEL_LAYER_ID_MAX_SIZE,
                    vec(1.0f, 1.0f),
                    color_scale(color_invert(color),
        ...
        if (active && label_layer->selection == (int)i) {
            Rect selection =
                rect_pad(camera_rect(camera,
                             boundary_of_element(label_layer, i, position)),
                    LABEL_LAYER_SELECTION_THICCNESS * 0.5f);

            if (camera_draw_thicc_rect_screen(
                    camera, selection, color, LABEL_LAYER_SELECTION_THICCNESS)
                < 0) {
                return -1;
            }
        }
...
static int label_layer_element_at(LabelLayer *label_layer, Vec2f position)
{
    trace_assert(label_layer);
    ...
    for (int i = n - 1; i >= 0; --i) {
        if (rect_contains_point(
                boundary_of_element(label_layer, (size_t)i, positions[i]),
                position)) {
            return i;
        }
    }
...
static void label_layer_delete_selected_label(
    LabelLayer *label_layer, UndoHistory *undo_history)
{
    trace_assert(label_layer);
    trace_assert(label_layer->selection >= 0);

    LABEL_UNDO_PUSH(undo_history,
        create_label_undo_context(label_layer, LABEL_UNDO_DELETE));
...
static int label_layer_add_label(LabelLayer *label_layer,
    Vec2f position,
    Color color,
    const char *text,
    UndoHistory *undo_history)
{
    ...
    memcpy(dynarray_pointer_at(&label_layer->texts, n),
        text,
        min_size_t(LABEL_LAYER_ID_MAX_SIZE - 1, strlen(text)));

    LABEL_UNDO_PUSH(
        undo_history, create_label_undo_context(label_layer, LABEL_UNDO_ADD));

    return (int)n;
}

static void label_layer_swap_elements(
    LabelLayer *label_layer, size_t a, size_t b, UndoHistory *undo_history)
{
    trace_assert(label_layer);
    ...
    LABEL_UNDO_PUSH(
        undo_history, create_label_undo_swap_context(label_layer, a, b));
}

static int label_layer_idle_event(LabelLayer *label_layer,
    ...
    UndoHistory *undo_history)
{
    ...
    int changed = 0;
    if (color_picker_event(&label_layer->color_picker, event, camera, &changed)
        < 0) {
        return -1;
    }
    ...
    case SDL_MOUSEBUTTONDOWN: {
        ...
        case SDL_BUTTON_LEFT: {
            const Vec2f position =
                camera_map_screen(camera, event->button.x, event->button.y);

            const int element = label_layer_element_at(label_layer, position);

            if (element >= 0) {
                label_layer->move_anchor =
                    vec_sub(position, positions[element]);
                ...
                label_layer->selection = label_layer_add_label(label_layer,
                    position,
                    color_picker_rgba(&label_layer->color_picker),
                    "",
                    undo_history);
                label_layer->state = LABEL_LAYER_EDIT_TEXT;
                edit_field_replace(&label_layer->edit_field,
                    texts + label_layer->selection * LABEL_LAYER_TEXT_MAX_SIZE);
                edit_field_restyle(&label_layer->edit_field,
                    LABELS_SIZE,
    ...
    case SDL_KEYDOWN: {
        ...
        case SDLK_UP: {
            if ((event->key.keysym.mod & KMOD_SHIFT)
                ...
                && ((size_t)(label_layer->selection + 1)
                       < label_layer->positions.count)) {
                label_layer_swap_elements(label_layer,
                    (size_t)label_layer->selection,
                    (size_t)label_layer->selection + 1,
                    undo_history);
        ...
        case SDLK_DELETE: {
            if (label_layer->selection >= 0) {
                label_layer_delete_selected_label(label_layer, undo_history);
                label_layer->selection = -1;
            }
        } break;
        ...
        case SDLK_c: {
            if ((event->key.keysym.mod & KMOD_LCTRL)
                && label_layer->selection >= 0) {
                label_clipboard = 1;
                dynarray_copy_to(&label_layer->texts,
                    label_clipboard_text,
                    (size_t)label_layer->selection);
                dynarray_copy_to(&label_layer->colors,
        ...
        case SDLK_v: {
            if ((event->key.keysym.mod & KMOD_LCTRL) && label_clipboard) {
                ...
                label_layer_add_label(label_layer,
                    position,
                    label_clipboard_color,
                    label_clipboard_text,
                    undo_history);
...
static void snap_inter_position(LabelLayer *label_layer, float snap_threshold)
{
    trace_assert(label_layer);
    trace_assert(label_layer->selection >= 0);
    ...
    Rect a = boundary_of_element(label_layer,
        (size_t)label_layer->selection,
        label_layer->inter_position);

    for (size_t i = 0; i < n; ++i) {
        if (i == (size_t)label_layer->selection)
            continue;
        ...
        if (segment_overlap(vec(a.x, a.x + a.w), vec(b.x, b.x + b.w))) {
            snap_seg2seg(
                &label_layer->inter_position.y, b.y, a.h, b.h, snap_threshold);
        }

        if (segment_overlap(vec(a.y, a.y + a.h), vec(b.y, b.y + b.h))) {
            snap_seg2seg(
                &label_layer->inter_position.x, b.x, a.w, b.w, snap_threshold);
        }
    }
}

static int label_layer_move_event(LabelLayer *label_layer,
    const SDL_Event *event,
    const Camera *camera,
    UndoHistory *undo_history)
{
    ...
    case SDL_MOUSEMOTION: {
        ...
        const Vec2f mouse_pos =
            vec_sub(camera_map_screen(camera, event->motion.x, event->motion.y),
                label_layer->move_anchor);
    ...
    case SDL_MOUSEBUTTONUP: {
        ...
        case SDL_BUTTON_LEFT: {
            const float distance =
                vec_length(vec_sub(label_layer->inter_position,
                    positions[label_layer->selection]));

            if (distance > 1e-6) {
                LABEL_UNDO_PUSH(undo_history,
                    create_label_undo_context(label_layer, LABEL_UNDO_UPDATE));
...
static int label_layer_edit_text_event(LabelLayer *label_layer,
    const SDL_Event *event,
    const Camera *camera,
    UndoHistory *undo_history)
{
    ...
    case SDL_KEYDOWN: {
        ...
        case SDLK_RETURN: {
            ...
            char *text = (char *)label_layer->texts.data
                + label_layer->selection * LABEL_LAYER_TEXT_MAX_SIZE;
            memset(text, 0, LABEL_LAYER_TEXT_MAX_SIZE);
            memcpy(text,
                edit_field_as_text(&label_layer->edit_field),
...
static int label_layer_edit_id_event(LabelLayer *label_layer,
    const SDL_Event *event,
    const Camera *camera,
    UndoHistory *undo_history)
{
    ...
    case SDL_KEYDOWN: {
        ...
        case SDLK_RETURN: {
            ...
            char *id = (char *)label_layer->ids.data
                + label_layer->selection * LABEL_LAYER_ID_MAX_SIZE;
            memset(id, 0, LABEL_LAYER_ID_MAX_SIZE);
            memcpy(id,
                edit_field_as_text(&label_layer->edit_field),
                LABEL_LAYER_ID_MAX_SIZE - 1);
            label_layer->state = LABEL_LAYER_IDLE;
            SDL_StopTextInput();
            return 0;
        } break;
...
static int label_layer_recolor_event(LabelLayer *label_layer,
    const SDL_Event *event,
    const Camera *camera,
    UndoHistory *undo_history)
{
    ...
    if (changed) {
        ...
        if (!color_picker_drag(&label_layer->color_picker)) {
            ...
            dynarray_replace_at(&label_layer->colors,
                (size_t)label_layer->selection,
                &label_layer->inter_color);
            label_layer->state = LABEL_LAYER_IDLE;
        }
    ...
    UndoHistory *undo_history)
{
    ...
    case LABEL_LAYER_EDIT_TEXT:
        return label_layer_edit_text_event(
            label_layer, event, camera, undo_history);

    case LABEL_LAYER_EDIT_ID:
        return label_layer_edit_id_event(
            label_layer, event, camera, undo_history);

    case LABEL_LAYER_RECOLOR:
        return label_layer_recolor_event(
            label_layer, event, camera, undo_history);
    }
