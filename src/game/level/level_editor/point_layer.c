...
static PointUndoContext create_point_undo_swap_context(
    PointLayer *point_layer, size_t index, size_t index2)
{
...
static PointUndoContext create_point_undo_context(
    PointLayer *point_layer, PointUndoType type)
{
    ...
    size_t index = type == POINT_UNDO_ADD ? point_layer->positions.count - 1
...
static void point_layer_undo(void *context, size_t context_size)
{
    ...
    case POINT_UNDO_DELETE: {
        dynarray_insert_before(&point_layer->positions,
            undo_context->index,
            &undo_context->position);
        dynarray_insert_before(
            &point_layer->colors, undo_context->index, &undo_context->color);
        dynarray_insert_before(
            &point_layer->ids, undo_context->index, &undo_context->id);
        point_layer->selection = -1;
    } break;

    case POINT_UNDO_UPDATE: {
        dynarray_replace_at(&point_layer->positions,
            undo_context->index,
            &undo_context->position);
        dynarray_replace_at(
            &point_layer->colors, undo_context->index, &undo_context->color);
    ...
    case POINT_UNDO_SWAP: {
        dynarray_swap(
            &point_layer->positions, undo_context->index, undo_context->index2);
        dynarray_swap(
            &point_layer->colors, undo_context->index, undo_context->index2);
        dynarray_swap(
            &point_layer->ids, undo_context->index, undo_context->index2);
    } break;
    }
}
...
#define POINT_UNDO_PUSH(HISTORY, CONTEXT)                                      \
    do {                                                                       \
        ...
        undo_history_push(                                                     \
            HISTORY, point_layer_undo, &context, sizeof(context));             \
    } while (0)

LayerPtr point_layer_as_layer(PointLayer *point_layer)
{
    LayerPtr layer = { .type = LAYER_POINT, .ptr = point_layer };
    return layer;
}
...
void point_layer_load(PointLayer *point_layer, Memory *memory, String *input)
{
    trace_assert(point_layer);
    trace_assert(memory);
    trace_assert(input);
...
static inline Triangle element_shape(Vec2f position, float scale)
{
    return triangle_mat3x3_product(equilateral_triangle(),
        mat3x3_product(trans_mat_vec(position), scale_mat(scale)));
}

int point_layer_render(
    const PointLayer *point_layer, const Camera *camera, int active)
{
    trace_assert(point_layer);
    trace_assert(camera);
    ...
    for (int i = 0; i < n; ++i) {
        const Color color =
            color_scale(point_layer->state == POINT_LAYER_RECOLOR
                        && i == point_layer->selection
                    ? point_layer->inter_color
                    : colors[i],
                rgba(1.0f, 1.0f, 1.0f, active ? 1.0f : 0.5f));

        const Vec2f position = point_layer->state == POINT_LAYER_MOVE
                && i == point_layer->selection
        ...
        if (active && i == point_layer->selection) {
            if (camera_fill_triangle(camera,
                    element_shape(position, POINT_LAYER_ELEMENT_RADIUS + 5.0f),
                    color_invert(color))
                < 0) {
                return -1;
            }
        ...
        if (camera_fill_triangle(camera,
                element_shape(position, POINT_LAYER_ELEMENT_RADIUS),
                color)
            < 0) {
            return -1;
        }
    ...
    if (point_layer->state == POINT_LAYER_EDIT_ID) {
        if (edit_field_render_world(&point_layer->edit_field,
                camera,
...
static int point_layer_element_at(const PointLayer *point_layer, Vec2f position)
{
    trace_assert(point_layer);
    ...
    for (int i = n - 1; i >= 0; --i) {
        if (vec_length(vec_sub(positions[i], position))
...
static int point_layer_add_element(PointLayer *point_layer,
    Vec2f position,
    Color color,
    UndoHistory *undo_history)
{
    ...
    POINT_UNDO_PUSH(
        undo_history, create_point_undo_context(point_layer, POINT_UNDO_ADD));

    return 0;
}

static void point_layer_swap_elements(
    PointLayer *point_layer, size_t a, size_t b, UndoHistory *undo_history)
{
    trace_assert(point_layer);
    trace_assert(undo_history);
    ...
    POINT_UNDO_PUSH(
        undo_history, create_point_undo_swap_context(point_layer, a, b));
}

static void point_layer_delete_nth_element(
    PointLayer *point_layer, size_t i, UndoHistory *undo_history)
{
    trace_assert(point_layer);

    POINT_UNDO_PUSH(undo_history,
        create_point_undo_context(point_layer, POINT_UNDO_DELETE));
...
static int point_layer_idle_event(PointLayer *point_layer,
    ...
    UndoHistory *undo_history)
{
    ...
    int selected = 0;
    if (color_picker_event(&point_layer->color_picker, event, camera, &selected)
        < 0) {
        return -1;
    }
    ...
    case SDL_MOUSEBUTTONDOWN: {
        ...
        case SDL_BUTTON_LEFT: {
            ...
            point_layer->selection =
                point_layer_element_at(point_layer, position);

            if (point_layer->selection < 0) {
                point_layer_add_element(point_layer,
                    position,
                    color_picker_rgba(&point_layer->color_picker),
                    undo_history);
            } else {
                ...
                point_layer->color_picker = create_color_picker_from_rgba(
                    colors[point_layer->selection]);
    ...
    case SDL_KEYDOWN: {
        ...
        case SDLK_UP: {
            if ((event->key.keysym.mod & KMOD_SHIFT)
                ...
                && ((size_t)(point_layer->selection + 1)
                       < point_layer->positions.count)) {
                point_layer_swap_elements(point_layer,
                    (size_t)point_layer->selection,
                    (size_t)point_layer->selection + 1,
                    undo_history);
                point_layer->selection++;
        ...
        case SDLK_DELETE: {
            if (0 <= point_layer->selection
                && point_layer->selection < (int)point_layer->positions.count) {
                point_layer_delete_nth_element(
                    point_layer, (size_t)point_layer->selection, undo_history);
                point_layer->selection = -1;
            }
        } break;
        ...
        case SDLK_F2: {
            if (point_layer->selection >= 0) {
                ...
                edit_field_replace(&point_layer->edit_field,
        ...
        case SDLK_c: {
            if ((event->key.keysym.mod & KMOD_LCTRL)
                && point_layer->selection >= 0) {
                point_clipboard = 1;
                dynarray_copy_to(&point_layer->colors,
        ...
        case SDLK_v: {
            if ((event->key.keysym.mod & KMOD_LCTRL) && point_clipboard) {
                ...
                point_layer_add_element(
                    point_layer, position, point_clipboard_color, undo_history);
            }
        } break;
        }
    } break;
    ...
    UndoHistory *undo_history)
{
    ...
    case SDL_KEYDOWN: {
        ...
        case SDLK_RETURN: {
            POINT_UNDO_PUSH(undo_history,
                create_point_undo_context(point_layer, POINT_UNDO_UPDATE));

            char *id = dynarray_pointer_at(
                &point_layer->ids, (size_t)point_layer->selection);
...
static int point_layer_move_event(PointLayer *point_layer,
    const SDL_Event *event,
    const Camera *camera,
    UndoHistory *undo_history)
{
    ...
    case SDL_MOUSEBUTTONUP: {
        ...
        case SDL_BUTTON_LEFT: {
            ...
            const float distance =
                vec_length(vec_sub(point_layer->inter_position,
                    positions[point_layer->selection]));
...
static int point_layer_recolor_event(PointLayer *point_layer,
    const SDL_Event *event,
    const Camera *camera,
    UndoHistory *undo_history)
{
    ...
    if (selected) {
        ...
        if (!color_picker_drag(&point_layer->color_picker)) {
            ...
            dynarray_replace_at(&point_layer->colors,
                (size_t)point_layer->selection,
                &point_layer->inter_color);
...
int point_layer_dump_stream(const PointLayer *point_layer, FILE *filedump)
{
    trace_assert(point_layer);
