...
inline static float layer_picker_max_width(void)
{
    ...
    return (float)max * FONT_CHAR_WIDTH * LAYER_TITLE_SIZE
        + LAYER_TITLE_PADDING * 2.0f;
}

#define LAYER_CELL_WIDTH layer_picker_max_width()
#define LAYER_CELL_HEIGHT                                                      \
    (LAYER_TITLE_SIZE * FONT_CHAR_HEIGHT + LAYER_TITLE_PADDING * 2.0f)
...
int layer_picker_render(const LayerPicker *layer_picker, const Camera *camera)
{
    ...
    for (size_t i = 0; i < LAYER_PICKER_N; ++i) {
        const Vec2f size = { .x = viewport.w * LAYER_CELL_WR_RATIO,
            .y = viewport.w * LAYER_CELL_WR_RATIO * LAYER_CELL_HW_RATIO };

        Vec2f position = { .x = 0.0f,
        ...
        if (camera_fill_rect_screen(camera,
                rect(
                    position.x, size.y * (float)i + position.y, size.x, size.y),
                color)
            < 0) {
            return -1;
        }
        ...
        camera_render_text_screen(camera,
            LAYER_CELL_TITLES[i],
            vec(size.x * LAYER_TITLE_SW_RATIO, size.x * LAYER_TITLE_SW_RATIO),
            color_invert(color),
            vec(position.x + size.x * LAYER_TITLE_PW_RATIO,
                size.y * (float)i + position.y
                    + size.x * LAYER_TITLE_PW_RATIO));
    }
