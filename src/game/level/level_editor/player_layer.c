...
static PlayerUndoContext player_layer_create_undo_context(
    PlayerLayer *player_layer)
{
    PlayerUndoContext context = { .layer = player_layer,
...
static void player_layer_undo(void *context, size_t context_size)
{
...
{
    LayerPtr layer = { .type = LAYER_PLAYER, .ptr = player_layer };
    return layer;
}

int player_layer_render(
    const PlayerLayer *player_layer, const Camera *camera, int active)
{
    trace_assert(player_layer);
    trace_assert(camera);

    if (camera_fill_rect(camera,
            rect_from_vecs(player_layer->position, vec(25.0f, 25.0f)),
            color_scale(color_picker_rgba(&player_layer->color_picker),
    ...
    UndoHistory *undo_history)
{
    ...
    if (color_picker_event(
            &player_layer->color_picker, event, camera, &selected)
        < 0) {
        return -1;
    }
    ...
    if (selected && !color_picker_drag(&player_layer->color_picker)) {
        ...
        undo_history_push(
            undo_history, player_layer_undo, &context, sizeof(context));
        player_layer->prev_color =
            color_picker_rgba(&player_layer->color_picker);
    }

    if (!selected && event->type == SDL_MOUSEBUTTONDOWN
...
int player_layer_dump_stream(const PlayerLayer *player_layer, FILE *filedump)
{
    ...
    fprintf(
        filedump, "%f %f ", player_layer->position.x, player_layer->position.y);
    color_hex_to_stream(
        color_picker_rgba(&player_layer->color_picker), filedump);
    fprintf(filedump, "\n");
