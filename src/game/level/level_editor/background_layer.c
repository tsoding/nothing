...
{
    BackgroundLayer layer = { .color_picker =
                                  create_color_picker_from_rgba(color),
        .prev_color = color };
    return layer;
}
...
static void background_undo_color(void *context, size_t context_size)
{
    ...
    UndoHistory *undo_history)
{
    ...
    if (color_picker_event(&layer->color_picker, event, camera, &selected)
        < 0) {
        return -1;
    }

    if (selected && !color_picker_drag(&layer->color_picker)) {
        BackgroundUndoContext context = { .layer = layer,
            .color = layer->prev_color };

        undo_history_push(
            undo_history, background_undo_color, &context, sizeof(context));
...
int background_layer_dump_stream(BackgroundLayer *layer, FILE *stream)
{
    ...
    color_hex_to_stream(color_picker_rgba(&layer->color_picker), stream);
