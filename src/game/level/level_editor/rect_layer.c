...
static RectUndoContext create_rect_undo_add_context(
    RectLayer *layer, size_t index)
{
    trace_assert(layer);
...
static RectUndoContext create_rect_undo_element_context(RectLayer *layer)
{
    trace_assert(layer);
...
static RectUndoContext create_rect_undo_update_context(RectLayer *rect_layer)
{
...
static RectUndoContext create_rect_undo_delete_context(RectLayer *rect_layer)
{
    RectUndoContext undo_context = create_rect_undo_element_context(rect_layer);
...
static RectUndoContext create_rect_undo_swap_context(
    RectLayer *rect_layer, size_t index1, size_t index2)
{
    RectUndoContext undo_context;
...
static void rect_layer_undo(void *context, size_t context_size)
{
    ...
    case RECT_UNDO_DELETE: {
        ...
        dynarray_insert_before(&layer->rects,
            undo_context->element.index,
            &undo_context->element.rect);
        dynarray_insert_before(&layer->colors,
            undo_context->element.index,
            &undo_context->element.color);
        dynarray_insert_before(&layer->ids,
            undo_context->element.index,
            &undo_context->element.id);
        dynarray_insert_before(&layer->actions,
            undo_context->element.index,
            &undo_context->element.action);
        layer->selection = -1;
    } break;

    case RECT_UNDO_UPDATE: {
        RectLayer *layer = undo_context->element.layer;
        dynarray_replace_at(&layer->rects,
            undo_context->element.index,
            &undo_context->element.rect);
        dynarray_replace_at(&layer->colors,
            undo_context->element.index,
            &undo_context->element.color);
        dynarray_replace_at(&layer->ids,
            undo_context->element.index,
            &undo_context->element.id);
        dynarray_replace_at(&layer->actions,
            undo_context->element.index,
            &undo_context->element.action);
    } break;
    ...
    case RECT_UNDO_SWAP: {
        RectLayer *layer = undo_context->element.layer;
        dynarray_swap(&layer->rects,
            undo_context->swap.index1,
            undo_context->swap.index2);
        dynarray_swap(&layer->colors,
            undo_context->swap.index1,
            undo_context->swap.index2);
        dynarray_swap(
            &layer->ids, undo_context->swap.index1, undo_context->swap.index2);
        dynarray_swap(&layer->actions,
            undo_context->swap.index1,
            undo_context->swap.index2);
    } break;
    }
...
#define RECT_UNDO_PUSH(HISTORY, CONTEXT)                                       \
    do {                                                                       \
        ...
        undo_history_push(                                                     \
            HISTORY, rect_layer_undo, &context, sizeof(context));              \
    } while (0)

static int rect_layer_add_rect(
    RectLayer *layer, Rect rect, Color color, UndoHistory *undo_history)
{
    trace_assert(layer);
    ...
    RECT_UNDO_PUSH(undo_history,
        create_rect_undo_add_context(layer, layer->rects.count - 1));
...
static void rect_layer_swap_elements(
    RectLayer *layer, size_t a, size_t b, UndoHistory *undo_history)
{
    trace_assert(layer);
...
static int rect_layer_delete_rect_at(
...
static int calc_resize_mask(Vec2f point, Rect rect)
{
    ...
    for (Rect_side side = 0; side < RECT_SIDE_N; ++side) {
        if (rect_side_distance(rect, point, side)
    ...
    UndoHistory *undo_history)
{
    ...
    int color_changed = 0;
    if (color_picker_event(&layer->color_picker, event, camera, &color_changed)
        < 0) {
        return -1;
    }
    ...
    if (color_changed) {
        if (layer->selection >= 0) {
            dynarray_copy_to(
                &layer->colors, &layer->inter_color, (size_t)layer->selection);
    ...
    case SDL_MOUSEBUTTONDOWN: {
        ...
        case SDL_BUTTON_LEFT: {
            ...
            int rect_at_position = rect_layer_rect_at(layer, position);

            Color *colors = (Color *)layer->colors.data;

            if (layer->selection >= 0 && layer->selection == rect_at_position
    ...
    case SDL_KEYDOWN: {
        ...
        case SDLK_UP: {
            if ((event->key.keysym.mod & KMOD_SHIFT) && (layer->selection >= 0)
                && ((size_t)(layer->selection + 1) < layer->rects.count)) {
                rect_layer_swap_elements(layer,
                    (size_t)layer->selection,
        ...
        case SDLK_v: {
            if ((event->key.keysym.mod & KMOD_LCTRL) && rect_clipboard) {
                ...
                rect_layer_add_rect(layer,
                    rect(position.x,
                        position.y,
                        rect_clipboard_rect.w,
                        rect_clipboard_rect.h),
                    rect_clipboard_color,
                    undo_history);
            }
        } break;
    ...
    case SDL_MOUSEBUTTONUP: {
        ...
        case SDL_BUTTON_LEFT: {
            const Rect real_rect =
                rect_from_points(layer->create_begin, layer->create_end);
            ...
            if (area >= CREATE_AREA_THRESHOLD) {
                ...
                log_info("The area is too small %f. Such small box won't be "
                         "created.\n",
                    area);
            }
...
static void snap_rect_resize_if_enabled(
    ...
    UndoHistory *undo_history)
{
    ...
    case SDL_MOUSEMOTION: {
        ...
        case 1: { // TOP
            ...
            layer->inter_rect = rect_from_points(
                vec(a.x, a.y), rect_position2(rects[layer->selection]));
        } break;
        ...
        case 4: { // BOTTOM
            ...
            layer->inter_rect =
                rect_from_points(rect_position(rects[layer->selection]),
        ...
        case 6: { // BOTTOM,LEFT
            ...
            layer->inter_rect =
                rect_from_points(vec(a.x, rects[layer->selection].y),
                    vec(rects[layer->selection].x + rects[layer->selection].w,
                        a.y));
        } break;
        ...
        case 9: { // TOP,RIGHT
            ...
            layer->inter_rect =
                rect_from_points(vec(rects[layer->selection].x, a.y),
                    vec(a.x,
                        rects[layer->selection].y + rects[layer->selection].h));
        } break;
        ...
        case 12: { // BOTTOM,RIGHT
            ...
            layer->inter_rect = rect_from_points(
                rect_position(rects[layer->selection]), vec(a.x, a.y));
        } break;
        }
    ...
    case SDL_MOUSEBUTTONUP: {
        ...
        dynarray_replace_at(
            &layer->rects, (size_t)layer->selection, &layer->inter_rect);
    } break;
    }
...
static void snap_rect_move_if_enabled(
    RectLayer *layer, Rect *a, float snapping_threshold)
{
    trace_assert(a);
    trace_assert(layer);
    ...
    UndoHistory *undo_history)
{
    ...
    case SDL_MOUSEMOTION: {
        ...
        const Vec2f mouse_pos =
            vec_sub(camera_map_screen(camera, event->button.x, event->button.y),
                layer->move_anchor);
        ...
        snap_rect_move_if_enabled(
            layer, &layer->inter_rect, SNAPPING_THRESHOLD / camera->scale);
    } break;

    case SDL_MOUSEBUTTONUP: {
        layer->state = RECT_LAYER_IDLE;

        float distance = vec_length(vec_sub(rect_position(layer->inter_rect),
            rect_position(rects[layer->selection])));

        if (distance > 1e-6) {
            RECT_UNDO_PUSH(
                undo_history, create_rect_undo_update_context(layer));
            dynarray_replace_at(
                &layer->rects, (size_t)layer->selection, &layer->inter_rect);
        }
    } break;
    ...
    UndoHistory *undo_history)
{
    ...
    case SDL_KEYDOWN: {
        ...
        case SDLK_RETURN: {
            ...
            char *id =
                dynarray_pointer_at(&layer->ids, (size_t)layer->selection);
            memset(id, 0, ENTITY_MAX_ID_SIZE);
            memcpy(id,
                edit_field_as_text(&layer->id_edit_field),
...
{
    LayerPtr layer = { .type = LAYER_RECT, .ptr = rect_layer };
    return layer;
}
    ...
    Memory *memory, const char *id_name_prefix, Cursor *cursor)
{
    ...
    rect_layer->ids =
        create_dynarray(memory, sizeof(char) * ENTITY_MAX_ID_SIZE);
    ...
    rect_layer->color_picker =
        create_color_picker_from_rgba(rgba(1.0f, 0.0f, 0.0f, 1.0f));
...
void rect_layer_load(RectLayer *layer, Memory *memory, String *input)
{
    ...
    for (int i = 0; i < n; ++i) {
        ...
        Action action = { .type = ACTION_NONE, .entity_id = { 0 } };

        String action_string = trim(chop_word(&line));
        if (action_string.count > 0) {
            action.type =
                (ActionType)atol(string_to_cstr(memory, action_string));
            switch (action.type) {
            case ACTION_NONE:
                break;
            ...
            case ACTION_HIDE_LABEL: {
                ...
                memcpy(action.entity_id,
                    label_id.data,
                    min_size_t(ENTITY_MAX_ID_SIZE - 1, label_id.count));
            } break;
...
int rect_layer_render(const RectLayer *layer, const Camera *camera, int active)
{
    ...
    if (active && layer->selection >= 0) {
        ...
        const Rect overlay_rect = rect_pad(
            camera_rect(camera, rect), -RECT_LAYER_SELECTION_THICCNESS * 0.5f);
        const Color overlay_color = color_invert(color);

        // Selection
        if (camera_draw_thicc_rect_screen(camera,
                overlay_rect,
                overlay_color,
                RECT_LAYER_SELECTION_THICCNESS)
            < 0) {
        ...
        const Vec2f rect_id_pos = vec_sub(rect_position(rect),
            vec_mult(RECT_LAYER_ID_LABEL_SIZE, vec(0.0f, FONT_CHAR_HEIGHT)));

        // Rectangle Id
        if (layer->state == RECT_LAYER_ID_RENAME) {
            // ID renaming Edit Field
            if (edit_field_render_world(
                    &layer->id_edit_field, camera, rect_id_pos)
                < 0) {
                return -1;
            }
        } else {
...
static int rect_layer_event_recolor(RectLayer *layer,
    const SDL_Event *event,
    const Camera *camera,
    UndoHistory *undo_history)
{
    ...
    if (color_changed) {
        ...
        if (!color_picker_drag(&layer->color_picker)) {
            RECT_UNDO_PUSH(
                undo_history, create_rect_undo_update_context(layer));
            dynarray_replace_at(
                &layer->colors, (size_t)layer->selection, &layer->inter_color);
            layer->state = RECT_LAYER_IDLE;
        }
    }
