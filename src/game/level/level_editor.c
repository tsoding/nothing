...
LevelEditor *create_level_editor(Memory *memory, Cursor *cursor)
{
    ...
    level_editor->edit_field_filename.font_color =
    ...
    level_editor->layers[LAYER_PICKER_BOXES] =
        rect_layer_as_layer(level_editor->boxes_layer);
    level_editor->layers[LAYER_PICKER_PLATFORMS] =
        rect_layer_as_layer(level_editor->platforms_layer);
    level_editor->layers[LAYER_PICKER_BACK_PLATFORMS] =
        rect_layer_as_layer(level_editor->back_platforms_layer);
    level_editor->layers[LAYER_PICKER_GOALS] =
        point_layer_as_layer(level_editor->goals_layer);
    level_editor->layers[LAYER_PICKER_PLAYER] =
        player_layer_as_layer(&level_editor->player_layer);
    level_editor->layers[LAYER_PICKER_LAVA] =
        rect_layer_as_layer(level_editor->lava_layer);
    level_editor->layers[LAYER_PICKER_REGIONS] =
        rect_layer_as_layer(level_editor->regions_layer);
    level_editor->layers[LAYER_PICKER_BACKGROUND] =
        background_layer_as_layer(&level_editor->background_layer);
    level_editor->layers[LAYER_PICKER_LABELS] =
        label_layer_as_layer(level_editor->label_layer);

    level_editor->notice = (FadingWigglyText) {
        .wiggly_text = { .text = "Level saved",
...
int level_editor_render(const LevelEditor *level_editor, const Camera *camera)
{
    ...
    if (camera_clear_background(camera,
    ...
    if (PLAYER_DEATH_LEVEL < world_viewport.y + world_viewport.h) {
        if (camera_fill_rect(camera,
                rect(world_viewport.x,
                    PLAYER_DEATH_LEVEL,
                    world_viewport.w,
                    world_viewport.h
                        + fmaxf(0.0f, world_viewport.y - PLAYER_DEATH_LEVEL)),
                LEVEL_EDITOR_DETH_LEVEL_COLOR)
            < 0) {
            return -1;
        }
    }
    ...
    for (size_t i = 0; i < LAYER_PICKER_N; ++i) {
        if (layer_render(level_editor->layers[i],
                camera,
                i == level_editor->layer_picker)
            < 0) {
            return -1;
        }
    }
    ...
    if (level_editor->state == LEVEL_EDITOR_SAVEAS) {
        ...
        if (edit_field_render_screen(&level_editor->edit_field_filename,
                camera,
                vec(position.x + save_as_width, position.y))
            < 0) {
            return -1;
        }
    }
    ...
    fading_wiggly_text_render(&level_editor->notice,
        camera,
...
static int level_editor_saveas_event(LevelEditor *level_editor,
...
static int level_editor_idle_event(
    LevelEditor *level_editor, const SDL_Event *event, Camera *camera)
{
    trace_assert(level_editor);
    trace_assert(event);
    trace_assert(camera);
    ...
    case SDL_MOUSEWHEEL: {
        ...
        if (event->wheel.y > 0) {
            ...
            level_editor->camera_scale =
                fmaxf(0.1f, level_editor->camera_scale - 0.1f);
        }
        camera_scale(camera, level_editor->camera_scale);
        Vec2f zoomed_position = camera_map_screen(camera, x, y);

        level_editor->camera_position = vec_sum(
            level_editor->camera_position, vec_sub(position, zoomed_position));
    ...
    case SDL_MOUSEMOTION: {
        if (level_editor->drag) {
            const Vec2f next_position =
                camera_map_screen(camera, event->motion.x, event->motion.y);
            const Vec2f prev_position = camera_map_screen(camera,
    ...
    if (layer_picker_event(
            &level_editor->layer_picker, event, camera, &selected)
        < 0) {
        return -1;
    }

    if (!selected) {
        if (layer_event(level_editor->layers[level_editor->layer_picker],
                event,
                camera,
...
static int level_editor_dump(LevelEditor *level_editor)
{
    ...
    for (size_t i = 0; i < LAYER_PICKER_N; ++i) {
        if (layer_dump_stream(
                level_editor->layers[level_format_layer_order[i]], filedump)
            < 0) {
            return -1;
        }
    }
