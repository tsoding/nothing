...
Explosion *create_explosion(Color color, float duration)
{
    Lt *lt = create_lt();
    ...
    explosion->pieces =
        PUSH_LT(lt, nth_calloc(1, sizeof(Piece) * EXPLOSION_PIECE_COUNT), free);
...
int explosion_render(const Explosion *explosion, const Camera *camera)
{
    trace_assert(explosion);
    trace_assert(camera);
    ...
    for (size_t i = 0; i < EXPLOSION_PIECE_COUNT; ++i) {
        ...
        color.a = fminf(1.0f,
            4.0f
                - (float)explosion->time_passed / (float)explosion->duration
                    * 4.0f);

        if (camera_fill_triangle(camera,
                triangle_mat3x3_product(explosion->pieces[i].body,
                    mat3x3_product(trans_mat(explosion->pieces[i].position.x,
...
int explosion_update(Explosion *explosion, float delta_time)
{
    trace_assert(explosion);
    ...
    for (size_t i = 0; i < EXPLOSION_PIECE_COUNT; ++i) {
        vec_add(&explosion->pieces[i].position,
            vec_scala_mult(explosion->pieces[i].direction, delta_time));
        explosion->pieces[i].angle = fmodf(explosion->pieces[i].angle
                + explosion->pieces[i].angle_velocity * delta_time,
            2.0f * PI);
    }
...
void explosion_start(Explosion *explosion, Vec2f position)
{
    ...
    for (size_t i = 0; i < EXPLOSION_PIECE_COUNT; ++i) {
        ...
        explosion->pieces[i].direction = vec_from_polar(
            rand_float_range(-PI, 0.0f), rand_float_range(100.0f, 300.0f));
    }
}
