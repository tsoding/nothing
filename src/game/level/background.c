...
static inline Vec2i chunk_of_point(Vec2f p)
{
    return vec2i((int)floorf(p.x / BACKGROUND_CHUNK_WIDTH),
        (int)floorf(p.y / BACKGROUND_CHUNK_HEIGHT));
}

int render_chunk(const Camera *camera, Vec2i chunk, Color color);

int background_render(const Background *background, const Camera *camera0)
{
    ...
    if (camera_clear_background(&camera, background->base_color) < 0) {
        return -1;
    }
    ...
    for (int l = 0; l < BACKGROUND_LAYERS_COUNT; ++l) {
        ...
        Vec2i max =
            chunk_of_point(vec_sum(position, vec(view_port.w, view_port.h)));

        for (int x = min.x - 1; x <= max.x; ++x) {
            for (int y = min.y - 1; y <= max.y; ++y) {
                if (render_chunk(&camera,
                        vec2i(x, y),
                        color_darker(
                            background->base_color, 0.05f * (float)(l + 1)))
                    < 0) {
                    return -1;
                }
            }
...
int render_chunk(const Camera *camera, Vec2i chunk, Color color)
{
    ...
    srand((unsigned int)(roundf(
        (float)chunk.x + (float)chunk.y + camera->scale * 10.0f)));
    ...
    for (size_t i = 0; i < BACKGROUND_TURDS_PER_CHUNK; ++i) {
        ...
        const float rect_w =
            rand_float_range(0.0f, BACKGROUND_CHUNK_WIDTH * 0.5f);
